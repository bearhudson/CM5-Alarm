/*
  ...
  ================================================================================
   ...
   Matrix Brightness: 
     - Night (sundown to sunrise): Intensity 1.
     - First hour after sunrise: Smoothly ramps from 1 to 5.
     - Daytime: Intensity 5.
   NeoPixels: 
...
const char* OPENWEATHER_KEY = "XXXXXX";
...
bool isDaytime                    = true;
uint8_t currentMatrixIntensity    = 0;
...
time_t updateClockData() {
  ...
  // 4. Evaluate Daylight vs Nighttime
...
// ===============================================================================
// --- Dynamic Matrix Brightness Ramp ---
// ===============================================================================
void updateMatrixBrightness(time_t now) {
  uint8_t targetIntensity = 1;
  ...
  if (sunriseEpoch > 0 && sunsetEpoch > 0) {
    if (now >= sunriseEpoch && now < sunsetEpoch) {
      ...
      if (secondsSinceSunrise < 3600) {
        // First hour after sunrise: Scale 1 -> 5
        targetIntensity = 1 + (uint8_t)((secondsSinceSunrise * 4) / 3600);
      } else {
        targetIntensity = 5;
      }
    } else {
...
void fetchEnvironmentData() {
  ...
  WiFiClient client;
  HTTPClient http;
  http.setTimeout(2500); // 2.5 second max timeout

  // 1. Current Weather Endpoint: Supplies accurate system 'now' timestamp, sunrise, sunset
  ...
  // 2. 5-Day / 3-Hour Forecast: Strict count limit (cnt=4) for stream parsing
